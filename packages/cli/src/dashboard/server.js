...
class DashboardServer {
    ...
    async start() {
        return new Promise((resolve, reject) => {
            ...
            this.server.listen(this.port, () => {
                console.log(chalk_1.default.green(`📊 Dashboard available at http://localhost:${this.port}`));
                resolve();
            });
    ...
    setupWebSocket() {
        ...
        this.wsServer.on('connection', (ws) => {
            ...
            const traceListener = (trace) => {
                ws.send(JSON.stringify({
                    type: 'trace:completed',
                    data: trace
                }));
            };
            const issueListener = (issue) => {
                ws.send(JSON.stringify({
