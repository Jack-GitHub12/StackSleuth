...
const mongoClient = new MongoClient(
  process.env.MONGO_URL || 'mongodb://localhost:27017/stacksleuth_demo'
);
...
app.post('/api/users', async (req, res) => {
  ...
  try {
    ...
    const existingUser = await instrumentedPool.query(
    ...
    // Create user
    const result = await instrumentedPool.query(
